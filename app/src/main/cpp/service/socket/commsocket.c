...
int sckServer_init(int port, int *listenfd) {
    ...
    if (ret < 0) {
        ret = errno;
        ALOGE("func setsockopt() err:%d \n", ret);
        return ret;
    }
    int recvbuff = 1*1024*1024;
    if(setsockopt(mylistenfd, SOL_SOCKET, SO_RCVBUF, (const char*)&recvbuff, sizeof(int)) == -1){
        ALOGE("setsocket recvbuff error\n");
    }
    else{
        ALOGE("setsocket recvbuff success\n");
    }
...
int sckServer_accept(int listenfd, int *connfd, int timeout) {
    ...
    if (ret < 0) {
        if (ret == -1 && errno == ETIMEDOUT) {
            ret = Sck_ErrTimeOut;
            ALOGE("func accept_timeout11() timeout err:%d \n", ret);
            return ret;
        } else {
            ret = errno;
            ALOGE("func accept_timeout22() err:%d \n", ret);
            return ret;
        }
    }
...
int sckServer_send(int connfd, unsigned char *data, int datalen, int timeout) {
    ...
    ALOGE("sckServer_send>> %d" , ret);
    if (ret == 0) {
        int writed = 0;
        unsigned char *netdata = (unsigned char *) malloc(datalen + 4);
        if (netdata == NULL) {
            ret = Sck_ErrMalloc;
            ALOGE("func sckServer_send() mlloc Err:%d\n ", ret);
            return ret;
        }
        int netlen = htonl(datalen);
        memcpy(netdata, &netlen, 4);
        ...
        writed = writen(connfd, netdata, datalen + 4);
        ALOGE("sckServer_send>> writed %d" , writed);
        if (writed <= (datalen + 4)) {
            if (netdata != NULL) {
                free(netdata);
                netdata = NULL;
...
int sckServer_rev(int connfd, unsigned char *out, int *outlen, int timeout) {
    ...
    if (out == NULL || outlen == NULL) {
        ret = Sck_ErrParam;
        ALOGE("func sckClient_rev()11 timeout , err:%d \n", Sck_ErrTimeOut);
        return ret;
    }

    ret = read_timeout(connfd, timeout); //bugs modify
    if (ret != 0) {
        if (ret == -1 || errno == ETIMEDOUT) {
            ret = Sck_ErrTimeOut;
            ALOGE("func sckClient_rev()22 timeout , err:%d  %d ", Sck_ErrTimeOut,errno);
            return ret;
        } else {
            ret = Sck_ErrTimeOut;//add zkk
            ALOGE("func sckClient_rev()33 timeout , err:%d  %d ", Sck_ErrTimeOut,errno);
            return ret;
        }
    }
